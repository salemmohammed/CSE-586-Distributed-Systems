...
public class OnPTestClickListener implements OnClickListener {
    ...
    private boolean testInsert() {
        ...
        int j = 0;
        Cursor cursor = sqLiteDatabase.rawQuery("select * from " + TABLE_MESSAGE + " where key=?", new String[] {"key49"});
        if(cursor.moveToFirst()) {
            do {
                Log.i("K FROM DATABASE ", cursor.getString(cursor.getColumnIndex("key")) + (++j));
                Log.i("V FROM DATABASE ", cursor.getString(cursor.getColumnIndex("value")) + (++j));
